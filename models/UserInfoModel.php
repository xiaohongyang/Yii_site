...
class UserInfoModel extends BaseModel
{
    ...
    public function getClockTimeHourConcatMinutes(){
        $hour = strlen($this->getAttribute('clock_time_hour'))==1 ? '0'.$this->getAttribute('clock_time_hour') : $this->getAttribute('clock_time_hour');
        $minutes = strlen($this->getAttribute('clock_time_minutes'))==1 ? '0'.$this->getAttribute('clock_time_minutes') : $this->getAttribute('clock_time_minutes');
        return $hour.':'.$minutes;
    }

    public function getOffDutyHourConcatMinutes(){
        $hour = strlen($this->getAttribute('off_duty_hour'))==1 ? '0'.$this->getAttribute('off_duty_hour') : $this->getAttribute('off_duty_hour');
        $minutes = strlen($this->getAttribute('off_duty_minutes'))==1 ? '0'.$this->getAttribute('off_duty_minutes') : $this->getAttribute('off_duty_minutes');
        return $hour.':'.$minutes;
    }
}