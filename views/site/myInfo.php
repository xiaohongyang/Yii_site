...
<div id="bg">
    ...
    ?>
    <input type="hidden"  name="<?=$model->formName()?>[homeLongitude]" id="homeLongitude" value="<?=$model->home_longitude?>" />
    <input type="hidden"  name="<?=$model->formName()?>[homeLatitude]" id="homeLatitude" value="<?=$model->home_latitude?>" />
    <input type="hidden"  name="<?=$model->formName()?>[companyLongitude]" id="companyLongitude" value="<?=$model->company_longitude?>" />
    <input type="hidden"  name="<?=$model->formName()?>[companyLatitude]" id="companyLatitude" value="<?=$model->company_latitude?>" />
    ...
    <div id="content">
        ...
        <?=$form->field($model, 'sex', ['options'=>[
            'class' => 'phone_no',
        ]])->radioList([1=>'男', 2=>'女'],['class'=>'tt inline', 'value'=>$model->getAttribute('sex')])->label('性&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;别：',['class'=>'sign'])->error(['class'=>'warning'])?>

        <?=$form->field($model, 'clockTime', ['options'=>[
            'class' => 'phone_no',
        ]])->textInput(['class'=>'tt', 'type'=>'time', 'value'=>$model->user->userInfo->clockTimeHourConcatMinutes])->label('上班时间：',['class'=>'sign'])->error(['class'=>'warning'])?>

        <?=$form->field($model, 'offDutyTime', ['options'=>[
            'class' => 'phone_no',
        ]])->textInput(['class'=>'tt', 'type'=>'time', 'value'=>$model->user->userInfo->offDutyHourConcatMinutes])->label('下班时间：',['class'=>'sign'])->error(['class'=>'warning'])?>

        <div class="xiaban_time">
            <span class="sign">家庭住址：</span>
            <input type="text" placeholder="请点击地图选择住址" name="<?=$model->formName()?>[homeAddress]" id="homeAddress" readonly="readonly" class="tt " value="<?=$model->home_address?>">
            <span class="bt_mp" onclick="setAddress(); return false;">地图</span>
        </div>
        <div class="shangban_time ex">
            <div class="fenge"></div>
            <span class="sign">公司住址：</span>
            <input type="text" placeholder="请点击地图选择住址" name="<?=$model->formName()?>[companyAddress]" id="companyAddress" readonly="readonly" class="tt" value="<?=$model->company_address?>">
        ...
        <?=$form->field($model, 'role', ['options'=>[
            'class' => 'jslx',
        ]])->radioList([1=>'乘客', 2=>'司机'],['class'=>'tt inline', 'value'=>$model->getAttribute('role')])->label('角色类型：',['class'=>'sign'])->error(['class'=>'warning'])?>

        <?=$form->field($model, 'timeliness', ['options'=>[
            'class' => 'intime',
        ]])->radioList([1=>'准时', 2=>'一般', 3=>'不准时'],['class'=>'tt inline', 'value'=>$model->getAttribute('timeliness')])->label('&nbsp;准&nbsp;时&nbsp;性：',['class'=>'sign'])->error(['class'=>'warning'])?>
...
<script>
    ...
    $(function() {
        ...
        $("input[name='tq_user_info[role]']").change(function() {
            if($(this).val()==2)
                $('#up_idcard').css('display','block');
        });
    })
